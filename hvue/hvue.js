...
class Compile {
  ...
  compileElement(node) {
    ...
    Array.from(node.attributes).forEach((attr) => {
      ...
      } else if (this.isEvent(attrName)) { // 如果是以@开头的事件
        // 拿到事件
        const eventName = attrName.substring(1);
        // 添加事件监听， 方法要注意this指向
        const enentFn = this.$vm.$options.methods[exp]
        node.addEventListener(eventName, enentFn.bind(this.$vm))
      }
    });
  }
